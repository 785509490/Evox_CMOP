...
from evox.operators.selection import  tournament_selection_multifit, nd_environmental_selection, dominate_relation_cons, dominate_relation, crowding_distance
...
class GMPEA(Algorithm):
    ...
    def init_step(self):
        ...
        if isinstance(fitness, tuple):
            ...
            _, _, self.dis = self.environmental_selection(self.pop, self.fit, None, self.pop_size, False)
    ...
    def step(self):
        ...
        if iscons:
            ...
            self.pop2, self.fit2, self.dis = self.environmental_selection(merge_pop2, merge_fit2, None, self.pop_size, False)
        else:
    ...
    def environmental_selection(self, PopX: torch.Tensor, PopObj: torch.Tensor, PopCon: torch.Tensor, N: int,
        ...
        if Next.sum().item() < N:
            ...
            if self.gen >= self.max_gen-2:
                ...
                mask = Next
                crowding_dis = crowding_distance(PopObj, mask)
                combined_order = lexsort([-crowding_dis])[:N]
                Next = combined_order
            # Del = self.truncation(PopObj[Next], Next.sum().item() - N)
            # Temp = Next.nonzero(as_tuple=True)[0]  # 获取需要删除的元素的索引
            # Next[Temp[Del]] = False
            # 找到 True 的索引
            # true_indices = Next.nonzero(as_tuple=True)[0]  # 获取 True 的索引
            # # 随机选择要置为 False 的索引
            # num_to_remove = Next.sum().item() - N  # 需要移除的 True 数量
            # # 在 true_indices 中随机选择 num_to_remove 个索引
            # indices_to_remove = torch.randperm(len(true_indices))[:num_to_remove]
            # # 置为 False
            # Next[true_indices[indices_to_remove]] = False
