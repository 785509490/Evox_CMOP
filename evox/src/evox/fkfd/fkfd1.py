...
def batch_update_radar(radar_channel, launch_missile, valid_mask,
    ...
    # 步骤4: 三维散射更新
    valid_indices = valid_mask.nonzero().squeeze(1)  # 有效样本索引 [V]
    # 初始化 usage 矩阵 [n, time_steps]
    usage = torch.zeros((radar_channel.size(0), radar_channel.size(1)), device=radar_channel.device)
    # 在时间区间内填充 radar_usage
    for i in valid_indices:
        start = start_idx[i]
        end = hit_idx[i]
        end = min(end, radar_channel.size(1) - 1)
        if start <= end:
            usage[i, start:end + 1] = radar_usage[i]
    # 生成雷达掩码 [n, radar_count]
    # 掩码作用：将 usage 的值仅广播到对应雷达的通道
    ...
    # 扩展 usage 到三维 [n, time_steps, radar_count]
    # 通过广播，将 usage 的值仅赋给当前雷达的通道
    usage_expanded = usage.unsqueeze(2) * radar_mask.unsqueeze(1)  # [n, time_steps, radar_count]
    ...
    if overflow_mask.any():
        ...
        # 标记这些样本为无效
        valid_mask[rollback_idx] = False  # 正确操作样本级索引
