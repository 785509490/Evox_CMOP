...
def dominate_relation_cons(x: torch.Tensor, y: torch.Tensor, consx: torch.Tensor, consy: torch.Tensor) -> torch.Tensor:
    ...
    max_obj = torch.max(x, dim=0, keepdim=True)[0].to(device)  # Shape (1, m)
    ...
    if violation_mask.any():
        ...
        x_upd[violation_mask] = max_obj + cv[violation_mask]  # Update infeasible individuals
    ...
    if violation_mask.any():  # If there are violations in y as well, use max_obj for updates
        ...
        y_upd[violation_mask] = max_obj + cv[violation_mask]  # Update infeasible individuals
